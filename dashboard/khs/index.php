...
if($role == "mahasiswa"){
    ...
    $query = "SELECT krs.id,mahasiswa.nama,prodi.fakultas,matkul.nama_matkul FROM krs JOIN mahasiswa ON mahasiswa.id = krs.mhs_id JOIN prodi ON prodi.id = krs.prodi_id JOIN matkul ON matkul.id = krs.matkul_id WHERE mahasiswa.user_id=$idUser ";
}else{
...
{ 
    ...
    <tr>
      ...
      <td><a class="btn btn-warning" href="<?= $baseUrl . '/dashboard/khs/edit.php?id=' .$row['id']  ?>" >edit</a></td>
      <td><a class="btn btn-danger" href="<?= $baseUrl . '/dashboard/khs/doDelete.php?id=' .$row['id'] ?>"  >delete</a></td>
      <?php }?>
    </tr>
    <?php  } ?>
  </tbody>
