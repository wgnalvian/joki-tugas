<?php
 ...
 $result = mysqli_query($connect,"SELECT * FROM jadwal_matkul WHERE id = $id");
...
<form method="POST" action="doAdd.php">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="mat">
                                        <?php while ($mat = mysqli_fetch_array($matFetch)) { ?>
                                            <option selected="<?= $now["matkul_id"] == $mat['id'] ? "selected" : "0" ?>"  value="<?= $mat["id"] ?>"><?= $mat["nama_matkul"] ?></option>
                                        <?php  } ?>
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="dos">
                                        <?php while ($dos = mysqli_fetch_array($dosFetch)) { ?>
                                            <option selected="<?= $now["dosen_id"] == $dos['id'] ? "selected" : "0" ?>" value="<?= $dos["id"] ?>"><?= $dos["nama"] ?></option>
                                        <?php  } ?>
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="ru">
                                        <?php while ($ru = mysqli_fetch_array($ruFetch)) { ?>
                                            <option selected="<?= $now["ruang_id"] == $ru['id'] ? "selected" : "0" ?>" value="<?= $ru["id"] ?>"><?= $ru["ruang"] ?></option>
                                        <?php  } ?>
                                    </select>
    ...
    <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="<?= $now["waktu"] ?>" placeholder="waktu" name="waktu">
